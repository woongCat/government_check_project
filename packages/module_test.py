...
if __name__ == "__main__": # 굳이 코드를 이런식으로 2번 하도록 만들 필요가 있을까 싶은데 # 일단은 한다 
    ...
    congress_speak_pdfs, congress_get_pdf_dates = get_api.get_pdf_url(congress_speak_url, congress_meeting_dates, unit_cd="22", page_size=100)
    logging.info("4-2 | Postgres에 위원회 pdf_url api 가져오기")
    committee_speak_pdfs, committee_get_pdf_dates = get_api.get_pdf_url(committee_speak_url, congress_meeting_dates, unit_cd="22", page_size=100)
